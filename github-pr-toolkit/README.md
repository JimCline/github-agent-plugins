...
|---|---|---|
| **`/resolve-pr-comments`** | Work through the review threads reviewers *already opened*: assess each, reply, fix or reject, and resolve — tracked as a task list. Assessment runs on a model you pick (default / Opus / Sonnet / Fable) — or hand it to a live peer agent (agent-hierarchy peer, agent-teams teammate, Herdr pane agent) rooted in the repo. | this file |
| **`/code-critic`** | *Author* an adversarial review of a local diff or a GitHub PR. The first question of every run declares the outcome — fix approved findings, or only comment/report them (the review itself is identical either way, and comment mode makes zero code changes) — then the review runs across user-selected categories (general, security, design, rules-adherence, performance, tests), via parallel per-category review subagents on a model you pick (session default / Opus / Sonnet / Fable, one across all categories — or the advisor / main agent). Or hand the review to a live peer agent (agent-hierarchy peer, agent-teams teammate, Herdr pane agent) rooted in the repo. High-signal by construction — every finding names what breaks if it ships, and finding nothing is a successful review. General Review also flags **ephemeral comments** — the `// changed from foo to bar` narration LLMs leave behind, which stops meaning anything once the diff merges. Severity-triaged findings tracked as a task list, nits batched into one ask, fix locally or post inline comments as one review — each opening with a colour-coded severity banner (🔴 CRITICAL … ⚪ NIT), in a configurable feedback tone (terse / balanced / suggestion). Review level (`--level low\|medium\|high`, default `medium`) governs what a non-material finding becomes. | [docs/code-critic.md](docs/code-critic.md) |
